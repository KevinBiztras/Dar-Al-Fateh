...
class AddEditAddress extends StatefulWidget {
  final String? addressEndpoint;

  const AddEditAddress({Key? key, this.addressEndpoint, }) : super(key: key);
...
class _AddEditAddressState extends State<AddEditAddress> {
  ...
  TextEditingController nameController = TextEditingController(text: AppSharedPref().getUserData()?.name);
        ...
        if (cou.name?.toLowerCase() == country?.toLowerCase()) {
          ...
          if ((cou.states ?? []).isNotEmpty) {
            ...
            if (selectedStateId == null) {
              setState(() {});
              print('country id 134-----> ${selectedCountryId??""}');
            }
          }
          break;
        }
