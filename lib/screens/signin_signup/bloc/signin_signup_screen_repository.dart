...
class SigninSignupScreenRepositoryImp implements SigninSignupScreenRepository {
  ...
  Future<SignUpScreenModel>? signUp(
    ...
    var deviceId = await PushNotificationsManager().createDeviceId();
    ...
    data['fcmDeviceId'] = deviceId;
    // data['is_seller'] = sellerDetails.becomeASeller;
    // if(sellerDetails.becomeASeller ?? false){
    //   data['url_handler'] = sellerDetails.profileUrl;
    //   data['country_id'] = sellerDetails.countryId;}
    String body = json.encode(data);
    SignUpScreenModel model = await ApiClient().getCustomerSignUp(body,0);
  ...
  Future<LoginResponseModel>? fingerPrintLogin(String loginKey) async {
    var model = await ApiClient().getCustomerLogIn(loginKey, "", "",0);
    if (model.success == true) {
  ...
  @override
  Future<CartViewModel> getMergeCart()async{
    CartViewModel model = await ApiClient().getMergeCart(AppSharedPref().getLoginKey() ?? "", AppSharedPref().getFcmToken());
    return model;
  }
