    ...
    return BlocBuilder<AccountInfoBloc, AccountInfoState>(
      ...
      if (currentState is AccountInfoLoadingState) {
        ...
        if (currentState.data.success ?? false) {
          ...
          if(changePassword && passwordController.text.trim().isNotEmpty) {
            AppSharedPref().setCurrentPassword(passwordController.text ?? "");
          }
        ...
        WidgetsBinding.instance?.addPostFrameCallback((_) {
          if (data.success == true) {
            ...
            AlertMessage.showError(
                _localizations?.translate(AppStringConstant.invalidPassword) ??
                    '',
                context);
          }
        });
      ...
      body: Stack(children: [
        SingleChildScrollView(
          child: Column(
            children: [
              accountInfoForm(),
              if (AppSharedPref().getSplashData()?.addons?.odooGdpr ?? false)
              ...
              if (AppSharedPref().getSplashData()?.addons?.odooGdpr ?? false)
                downloadInfo(),
              if ((AppSharedPref().getSplashData()?.addons?.odooGdpr ??
                      false) &&
                  !(AppSharedPref().getUserData()?.isEmailVerified ?? true))
                verificationContainer(context, _localizations, emailController,
                    () {
                  ...
                  if (AppSharedPref().getUserData()?.email !=
                    ...
                    DialogHelper.confirmationDialog(
                        _localizations?.translate(
                                AppStringConstant.deleteAccountAlert) ??
                            '',
                        context,
                        _localizations, onConfirm: () {
                      DialogHelper.deleteAccountConfirmationDialog(
                          context,
                          _localizations?.translate(
                                  AppStringConstant.deleteAccount) ??
                              "",
                          "${_localizations?.translate(AppStringConstant.deleteAccountWarningStartMessage)} ${AppSharedPref().getUserData()?.email} ${_localizations?.translate(AppStringConstant.deleteAccountWarningEndMessage)}",
                          (p0) {
                        _accountInfoBloc?.add(DeleteAccountLoginEvent(
                            AppSharedPref().getUserData()?.email ?? "", p0));
                      }, _localizations);
                    });
                  } else {
                    DialogHelper.showExceptionDialog(
                        _localizations?.translate(
                                AppStringConstant.dontHavePermission) ??
                            "",
                        context,
                        buttonText:
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  }, changePassword, colors: Theme.of(context).cardColor),
                  const SizedBox(height: AppSizes.normalPadding),
                  if (changePassword)
                    Column(
                      children: [
                    ...
                    if (_formKey.currentState?.validate() == true) {
                      if (changePassword) {
                        if (currentPasswordController.text ==
                            AppSharedPref().getCurrentPassword()) {
                          _accountInfoBloc?.add(SaveAccountInfoEvent(
                              nameController.text, passwordController.text));
                        } else {
                          AlertMessage.showError(
                              _localizations!.translate(
                                  AppStringConstant.passwordValidation),
                              context);
                        }
                      } else {
                        _accountInfoBloc?.add(SaveAccountInfoEvent(
