    ...
    return Scaffold(
      ...
      body: BlocBuilder<OrderScreenBloc, OrderScreenState>(
        ...
        if (currentState is OrderScreenInitial) {
          ...
          } else if ((orderList?.tcount ?? 0) != recentOrders.length) {
          ...
          RefreshIndicator(
            onRefresh: () {
              return Future.delayed(Duration.zero).then((value) {
                _orderScreenBloc?.add(OrderScreenDataFetchEvent(
                    0, (widget.isFromDashboard) ? 5 : 10));
              });
            },
            notificationPredicate: (notification) {
              return !(widget.isFromDashboard);
            },
            child: OrderMainView(
              orders: recentOrders,
          ...
          child: Center(
            child: lottieAnimation(
              ...
              lottiePath: (Theme.of(context).brightness == Brightness.light)
                  ...
                  : "lib/assets/lottie/empty_order_dark.json",
            ),
          ),
        ),
