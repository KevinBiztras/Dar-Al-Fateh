...
class PaymentReviewScreenBloc extends Bloc<PaymentReviewScreenEvent, PaymentReviewScreenState>{
  ...
  void mapEventToState(
    ...
    }else if(event is ApplyCouponCodeEvent){
      try {
        var model = await repository?.applyCouponCode(event.coupon);
        if (model != null) {
          emit( ApplyCouponCodeSuccessState(model));
        } else {
          emit(PaymentReviewScreenError(''));
        }
      } catch (error, _) {
    ...
    }else if(event is RemoveCouponCodeEvent){
      try {
        var model = await repository?.removeCouponCode(event.lineId);
        if (model != null) {
          emit( RemoveCouponCodeSuccessState(model));
        } else {
          emit(PaymentReviewScreenError(''));
        }
      } catch (error, _) {
